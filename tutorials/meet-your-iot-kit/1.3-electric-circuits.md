...
Some IoT devices won't need to use either power rail on the breadboard. However, most IoT devices will use one of the power rails.  A few IoT devices might need to use both power rails \(e.g., if your device has multiple parts needing 3.3V and multiple parts needing 5V\).
