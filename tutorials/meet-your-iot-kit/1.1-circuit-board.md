...
Once the Photon is powered on and connected to Wi-Fi, the Photon will automatically connect to Particle Cloud, which is a cloud service that Particle provides for all of its microcontroller devices.

Particle Cloud can be used to:

* Code and store all your different Photon device apps \(using the online Particle Build code editor\)
...
A breadboard can be used to connect multiple inputs and outputs to the same power supply pins. The third section of this tutorial will explain how this works.
...
* **RGB:** This LED indicates the Photon's device status using [different colors and blinking patterns](https://docs.particle.io/guide/getting-started/modes/photon/).
...
To indicate the Photon device's status, the RGB LED will change color & will either "blink" \(fast blinking\) or "breathe" \(slow blinking\).

### Normal RGB Patterns

The Photon circuit board does **not** have an "on/off" switch.  As soon as a power source is connected to the USB port or barrel jack, the Photon will power on.  To turn off the Photon, you have to disconnect its power source.

Whenever you connect a power source to the Photon \(or restart it using the Reset button\), the Photon will power on and try to connect to Wi-Fi and then to the Particle Cloud service \(which is used to update your device app and firmware & to communicate with your web app\)

| **RGB Color** | **RGB Pattern** | **Device Status** |
| --- | --- | --- | --- | --- |
| Green | Blinking | Trying to connect with Wi-Fi |
| Cyan \(pale blue\) | Blinking | Connected to Wi-Fi, Trying to connect with Particle Cloud |
| Cyan \(pale blue\) | Breathing | Connected to Wi-Fi and Particle Cloud |
| Pink | Blinking | Receiving new app or new firmware over Wi-Fi |
...
| Pink | Breathing | Safe Mode \(connected but does **not** run app\) |
| White | Breathing | Device's Wi-Fi module turned off |
| Orange-Yellow | Blinking | DFU Mode \(ready for manual device firmware upgrade\) |
| Red | Blinking | Firmware error |
