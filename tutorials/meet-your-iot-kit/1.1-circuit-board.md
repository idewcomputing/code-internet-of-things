...
* **Barrel Jack:**  A barrel jack adapter can be plugged in to provide power from an external supply such as a battery or outlet.
* **USB:**  A Micro-USB cable can be plugged in to provide power from a computer's USB port or a USB charger.
...
As this occurs, the RGB LED will change its color and blinking pattern to indicate the current status of the Photon device. The RGB LED will either "blink" \(fast blinking\) or "breathe" \(slow blinking\).
...
| Cyan \(pale blue\) | Breathing | Connected to Wi-Fi and Particle Cloud \(app running\) |
| Pink | Blinking | Receiving new device app or new firmware over Wi-Fi |

### Other RGB Patterns 

If your Photon device displays one of these other RGB patterns, then the Photon has encountered an issue or has been placed into a different mode.
...
| Pink | Breathing | Safe Mode \(does **not** run app\) |
