...
{% hint style="success" %}
**GOAL:**  Understand how to create electronic circuits by connecting components to the IoT circuit board \(use of power pins, I/O pins, jumper wires, and breadboard\)
...
Each input and output connected to your Photon circuit board must form an [electronic circuit](https://www.dummies.com/programming/electronics/components/what-is-an-electronic-circuit/), which is a continuous path that conducts low voltage electricity from the positive \(+\) end of a power source through an "active" component \(such as an input or output\) and back to the negative \(-\) end of the power source.

### Electronic vs. Electric

An electronic circuit is a special type of electric circuit. Here are two key differences:
...
Electronic circuits operate on **low voltage direct current** \(less than 15 volts\).
