...
Next, you'll add code to allow your web app JS file to interact with your Photon device through Particle Cloud. Remember that the web app will perform these two tasks:
...
The Particle API JS library defines a class of [objects](https://www.w3schools.com/js/js_objects.asp) called `Particle()` which has built-in methods to allow a web app to interact with a Photon device through Particle Cloud.
...
Add this JS code to your `code.js` file, and then modify it:
...
In your Photon device app, the `lightStatus` variable has a value of either `"on"` or `"off"`, so in your web app, `data.body.result` will also have one of these values.
...
* `$("#light1")` targets the HTML element with `id="light1"` which is the `<div>` element that represents the "card" displaying the information for the light
* `$("#light1-status")` targets the HTML element with `id="light1-status"` which is an `<h2>` element that lists whether the light is current on or off
* `$("#light1-button")` targets the HTML element with `id="light1-button"` which is the `<button>` element that can be clicked to remotely turn the light on or off
