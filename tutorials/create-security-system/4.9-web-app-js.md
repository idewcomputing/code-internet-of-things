...
In this case, JS and jQuery code statements \(lines 29-33 above\) were added that will be performed each time a new event notification is received:
...
Since your "Smart Security" Photon app is sending the `"motion"` event notification **without** data, there aren't any JS or jQuery code statements using `feed.data` \(because no data is being received\).
