...
Once your Photon has downloaded the app and restarted, the updated app will start running. You shouldn't notice any visible changes in the device's behavior, but your Photon app will now be sharing the `deviceMode` variable and `toggleMode()` function with Particle Cloud and will be sending `"motion"` event notifications to Particle Cloud when motion is detected while the device is armed.
